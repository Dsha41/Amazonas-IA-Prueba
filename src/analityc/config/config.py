...
# ---------------------------------------------------------------------------
# Rutas base resueltas RELATIVAS al root del proyecto para que funcionen tras
# clonar el repo en cualquier maquina. Estructura esperada:
#   <PROJECT_ROOT>/src/analityc/config/config.py
# (parents[3] sube 4 niveles desde este archivo para llegar al root).
# Se pueden sobreescribir con las ENV vars MODELS_DIR y OUTPUT_DIR.
# ---------------------------------------------------------------------------
...
def _build_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "model_paths": {
            ...
            # Reemplazado de 'amazonas.pt' -> cosmeticos/weights/best.pt
            # (16 SKUs retail). Se respeta env MODEL_PATH_AMAZONAS si se setea.
            "Personal de Amazonas": _env_str(
                "MODEL_PATH_AMAZONAS",
                str(Path(models_dir).parent / "cosmeticos" / "weights" / "best.pt"),
            ),
        },
        "person_model_paths": {
        ...
        # Bajados para mejorar recall del modelo cosmeticos (SKUs retail).
